...
import {
  ...
  updateRowData
} from '../../utils/helpers/contexts/redux/reducers/dataGridSlice'
...
const useDataGridHelper = () => {
  ...
  const { page, limit, offset } = useSelector(
    (state) => state.dataGridSlice.details
  )
  ...
  const tableColumns = [
    ...
    // Action button config
    {
      ...
      renderCell: (obj) => {
        return (
          <Button
            onClick={() => handleFetchCharacterDetails(obj.id)}
  ...
  const handlePageChange = (newPage) => {
    ...
    dispatch(
      updatePaginationParams({ page: newPage, limit, offset: newOffset })
    )
  }
  ...
  const handleFetchCharacterDetails = (characterId) => {
    if (characterId) {
      ...
      enqueueSnackbar({
        message: 'Character id is missing, try again !!',
        variant: 'error'
      })
    }
  }
...
const useDataGridAPIHelper = () => {
  ...
  const { limit, page, offset } = useSelector(
    (state) => state.dataGridSlice.details
  )
  // redux dispatch
  const dispatch = useDispatch()
  ...
  const getAllCharacters = () => {
    ...
    services
      .getAllCharacters({ limit, page, offset, dataGridConfig })
      .then((response) => {
